...
private void search()
{
    ...
    Query query=FirebaseDatabase.getInstance().getReference().child("Products").orderByChild("price").startAt(search1);
    query.addValueEventListener(postListener);
}
    private void search11()
    {
        String search1=searchet.getText().toString().trim();
        Query query=FirebaseDatabase.getInstance().getReference().child("Products").orderByChild("pname").startAt(search1);
        query.addValueEventListener(postListener);
    }
